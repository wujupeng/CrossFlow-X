...
> **文档状态**：DRAFT v1.7（Amendment，受控修订，不推倒 v1.6 主体，本次为 Contract Consistency Cleanup + Final Proof Closure）→ 待用户审查冻结（Evidence-First，先规格后实现；本设计仅覆盖 CF2-S01～S06 六个 macOS 输入捕获地基的增量设计方案 + 工程边界量化 + CF0 Safety Invariant 保障 + Verification Matrix 回映，不引入规格外能力，不修改 CF0/CF1 Frozen 文档，不进入 Task Design，不直接 Coding）
...
> **Amendment v1.7 变更记录（Contract Consistency Cleanup + Final Proof Closure，受控修订，不推倒 v1.6 主体，不继续扩充理论）**
> **修订背景**：大G 项目经理对 Design v1.6 的 Design Gate 终审裁决 = CONDITIONAL FAIL / REVISION REQUIRED。v1.6 声称已修复 R17/R18，但文档内部仍存在前后矛盾：R17 在文档层面没有真正闭环（前面的最终 Contract 中仍保留 v1.5 旧 INV7/INV8 定义 `droppedCount = effectiveTail - tail` 和 `head = droppedCount + tail + (head - effectiveTail)`，后面的 v1.6 Amendment 又重新定义成 logicalDropBoundary / pendingDropped / cumulativeDropCount，旧 INV7/INV8 没有被彻底从冻结 Contract/Verification Matrix 中清除，验证清单仍要求 INV7/INV8 同时又说 cumulativeDropCount 已经只是 observational metric）；R18 未完全闭环（P1-P8 方向正确，但还需确认核心语义 logicalDropBoundary = max(tail, head-Capacity) 是派生窗口而非实际线性化的 Drop 操作，必须证明 Producer Push LP → logical oldest position becomes expired → Consumer Pop LP 三者关系，特别是 Consumer 已读取 head → Producer 又 Push → head-Capacity 改变 → Consumer 使用此前看到的 head 这类 interleaving 的线性化结果）。本次 Amendment **不是继续扩充理论，而是做一次全文一致性清理 + 最终证明闭合**，仅修复下列 8 项，不改变主体结构 / 六个模块 / CF0-CF1 边界 / Driverless 原则 / 安全目标 / Requirements v1.4 FROZEN / R1～R18 已修复内容，不修改 CF0/CF1 Frozen 文档，不进入 Task Design，不直接 Coding。
> **修订项**：
> - **Design-R17-CONSISTENCY (🔴 BLOCKER，v1.6 → v1.7 彻底闭环)**：v1.6 在 R17 修复子节中引入了新命名 logicalDropBoundary / pendingDropped / cumulativeDropCount，但**没有彻底清除**正文 Contract / 不变量列表 / Verification Matrix 中的旧 INV7 `droppedCount = effectiveTail - tail` 和旧 INV8 `head = droppedCount + tail + (head - effectiveTail)` 定义——仅在旧定义旁边加 v1.6 修正注释（"本 INV7 原文保留为历史记录，最终语义以 INV7a/INV7b 为准"），导致全文存在两套语义。**修正**：(1) 从 §2.4.8 方案 A++ 不变量列表中**彻底删除**旧 INV7 / INV8 定义（不只是加注释），替换为 INV7a（pendingDropped 瞬时派生量）+ INV7b（cumulativeDropCount observational metric）+ 区间分解；(2) 从 §2.4.8 Design-R15 最终 Contract 不变量列表中彻底删除旧 INV7/INV8 引用；(3) 从 §2.4.8 可验证 invariant 审查清单中删除旧 INV7/INV8 引用；(4) 从 §2.6.8 Verification Matrix R7/R11/R16 行中删除旧 INV7/INV8 引用，统一引用 INV7a/INV7b + 区间分解；(5) 旧 INV7/INV8 仅在 Amendment v1.5/v1.6 变更记录中保留作为历史记录（标注"已废弃"），Contract / Invariant 列表 / Verification Matrix 正文中不出现。
> - **Design-R17-NAMING (🔴 BLOCKER，v1.7 全文统一命名)**：v1.6 在 R17 修复子节中使用新命名 logicalDropBoundary / pendingDropped / cumulativeDropCount，但正文其他位置（Contract、不变量列表、Consumer tryPop 代码、R16 修复子节、五种 Consumer-pause 状态分析、统计恒等式证明、Verification Matrix）仍使用旧名 effectiveTail / droppedCount。**修正**：全文正文位置统一使用 logicalDropBoundary（替代 effectiveTail）/ pendingDropped（替代 droppedCount 瞬时量语义）/ cumulativeDropCount（替代 droppedOldestCount 累计量语义）。旧名 effectiveTail 仅在 Amendment v1.5/v1.6 变更记录 + R17/R18 修复子节的历史回顾中保留（作为历史记录）。旧名 droppedCount 仅在 Amendment v1.5/v1.6 变更记录中保留（已废弃说明）。
> - **Design-R17-OBSERVATIONAL (🟠 P1，v1.7 cumulativeDropCount 明确为 observational metric)**：v1.6 在 R17 修复子节中声明 cumulativeDropCount 降级为 observational metric，但 §2.4.8 统计恒等式证明仍把 droppedOldestCount 作为"INV7 的 droppedCount 增量"参与 correctness 表述，Verification Matrix 仍要求 INV8 统计恒等式成立。**修正**：(1) 重写 §2.4.8 统计恒等式证明，明确 cumulativeDropCount 不参与 correctness 证明，仅作为 observational metric（监控告警 / 容量调优 / DFX 指标）；(2) correctness 由 seqlock double-read validation（R15）+ logicalDropBoundary 对齐（R16）+ 区间分解（R17-B）保证，不依赖 cumulativeDropCount 的精确值；(3) Verification Matrix 中不要求 cumulativeDropCount 满足任何 correctness 条件，仅要求其作为 std::atomic<uint64_t> 可观测（relaxed 读）。
> - **Design-R18-FOUR-ELEMENTS (🔴 BLOCKER，v1.7 P1-P8 每种情况明确四要素)**：v1.6 的 R18 P1-P8 线性化证明给出了 Producer LP / Consumer LP / Consumer 行为 / 正确性结论，但未明确**Drop LP / 语义发生点**（即 logical oldest position 何时失效）和 **Consumer 最终接受/拒绝原因**。**修正**：§2.4.8 R18 修复子节 P1-P8 表格增加两列：Drop LP / 语义发生点（定义 A：Producer 推进 head 到 position + Capacity 之后该 position 立即逻辑失效）+ Consumer 最终接受/拒绝原因（epoch 校验 + 覆写校验结果）。
> - **Design-R18-THEOREM (🔴 BLOCKER，v1.7 增加 Epoch Validity Linearization Theorem)**：v1.6 的 R18 核心结论只给出 `Lp < Lc ⇒ old item cannot be returned` 和 `Lc < Lp ⇒ old item may be returned legitimately`，但未形式化证明"已被 Producer Push LP 之前失效的 epoch 不得被 Consumer 在其 Pop LP 之后返回"。**修正**：§2.4.8 R18 修复子节末尾增加 **Theorem (Epoch Validity Linearization)**：形式化定义 Producer 写 slot i 的 LP 为 Lp(i)，Consumer 读 slot i 的 Pop LP 为 Lc(i)，证明：(1) 若 epoch(i) 在 Lp(j) 时已失效（j > i 且 j - i ≥ Capacity），则不存在合法线性化使 Consumer 在 Lc(i) > Lp(j) 后返回 item(i)；(2) 若 epoch(i) 在 Lc(i) 时仍有效（∀j: Lp(j) < Lc(i) ⇒ j < i + Capacity），则 Consumer 可合法返回 item(i)。
> - **Design-VERIFICATION-MATRIX-CONSISTENCY (🟠 P1，v1.7 Verification Matrix 与最终 Contract 逐项一致)**：v1.6 的 Verification Matrix 中 R7/R11/R16 行仍引用旧 INV7/INV8 和旧名 effectiveTail/droppedCount，与 v1.6 R17 修复子节的新命名不一致。**修正**：Verification Matrix 中每一行引用最终 Contract 中的具体不变量编号（INV1～INV6 + INV7a + INV7b + 区间分解），不引用已废弃的 INV7/INV8，不引用旧名 effectiveTail（除非在历史变更记录中）。
> - **Design-KEYWORD-AUDIT (🟠 P1，v1.7 全文关键词审计)**：v1.6 没有明确审计旧关键词的残留位置。**修正**：文档末尾增加 Keyword Audit 子节，明确列出 droppedCount / INV7 / INV8 / effectiveTail 各自的合法出现位置（仅 Amendment 变更记录中的历史记录），确保 Contract / Invariant 列表 / Verification Matrix / 代码示例 / 测试设计中不出现旧关键词。
> - **Design-CONTRACT-SINGLE-SEMANTICS (🟠 P1，v1.7 全文只有一套 Queue State Model)**：v1.6 在 Contract 中同时保留旧 INV7/INV8 和新 INV7a/INV7b + 区间分解，导致全文存在两套语义。**修正**：重写最终 Contract，使全文只有一套 Queue State Model，Contract 中的不变量列表只包含 INV1～INV6（原有）+ INV7a（pendingDropped）+ INV7b（cumulativeDropCount observational）+ 区间分解，旧 INV7/INV8 完全消失。
> **未变更项（v1.7）**：六个模块（CF2-S01～S06）、CF0-CF1 边界、Driverless User-Mode Architecture、CF0 Safety Invariant（P1/P2/P3）、CF0 七契约、CF0 8 线程模型、CF1 Node Identity、第一原则落地路径、Requirements v1.4 FROZEN 全部内容、双通道架构（SPSC_DATA=256 / SPSC_STATE=64）、STATE saturation safety path、CGEventSourceFlagsState ground truth、bitmap ownership（方案 B SPSC snapshot publication）、RECOVERY、R9 ≤100ms total deadline、R11 ownership model、Callback Boundary（R14）、Design-R1/R2/R3/R4/R5/R6/R8/R9/R10/R12/R13/R14/R15/R16/R17/R18 已修复内容全部保持不变。`SpscRingBuffer` 公开接口签名（`tryPush/tryPop/tryPushDropOldest`）不变。方案 A++ 的 word-atomic payload + seqlock double-read validation 核心算法不变（R15 成果保留）。SPSC ownership（head=Producer, tail=Consumer）不变。epoch seq 语义不变（`seq = h + 1`）。tryPush 满判定不变（`h - tail == Capacity`）。tryPushDropOldest 不判满、总是覆写不变。**v1.7 仅做 Contract Consistency Cleanup（彻底删除旧 INV7/INV8 + 全文统一命名 logicalDropBoundary/pendingDropped/cumulativeDropCount + cumulativeDropCount 明确为 observational metric + 重写最终 Contract 使全文只有一套语义）+ Final Proof Closure（R18 P1-P8 每种情况明确四要素 + 增加 Epoch Validity Linearization Theorem + Verification Matrix 与最终 Contract 逐项一致 + 全文 Keyword Audit），不改变 Producer 写入路径、不改变 seqlock double-read validation、不改变 word-atomic payload 存储、不改变 logicalDropBoundary 对齐步骤、不重新引入 publishedTail、不让 Producer 写 Consumer-owned tail、不重新设计整个队列、不继续扩充理论。**
...
**方案 A++ 不变量（v1.7 Contract Consistency Cleanup：废弃 INV2-OLD，引入 logicalDropBoundary 派生量，彻底删除旧 INV7/INV8，统一使用 INV7a/INV7b + 区间分解）**：

> **⚠️ Design-R16 修正（v1.5）+ Design-R17-CONSISTENCY 修正（v1.7）**：v1.4 的 INV2 `head - tail ≤ Capacity` 在 Drop-Oldest 模式下**不成立**（`tryPushDropOldest` 不判满、不推进 tail，Producer 连续写入后 `head - tail` 可远超 Capacity）。v1.5 废弃 INV2-OLD，引入**派生量 `logicalDropBoundary`**（v1.5 原名 `effectiveTail`，v1.7 全文统一重命名）重新定义 Queue Invariant。v1.6 进一步引入 `pendingDropped` / `cumulativeDropCount` 三量分离 + 区间分解替代旧 INV7/INV8。**v1.7 彻底从 Contract 中删除旧 INV7 `droppedCount = effectiveTail - tail` 和旧 INV8 `head = droppedCount + tail + (head - effectiveTail)`**（旧定义仅在 Amendment v1.5/v1.6 变更记录中保留作为历史记录）。**这是 invariant 重新定义 + 命名统一，不是算法改变**——方案 A++ 的 word-atomic payload + seqlock double-read validation 核心算法不变，Producer 写入路径不变，R15 成果保留。
...
- **INV2（v1.5 重新定义，Design-R16 核心修复，v1.7 命名统一）**：`head - logicalDropBoundary ≤ Capacity`（**逻辑占用 bounded，恒成立**）。其中 `logicalDropBoundary = max(tail, head - Capacity)` 为**派生量**（非存储的原子变量，由 Producer-owned `head` + Consumer-owned `tail` 数学关系定义）。**证明**：`logicalDropBoundary = max(tail, head - Capacity) ≥ head - Capacity`，故 `head - logicalDropBoundary ≤ head - (head - Capacity) = Capacity`。✓
...
- **INV6（logicalDropBoundary 定义，v1.5 Design-R16 新增，v1.7 命名统一）**：`logicalDropBoundary = max(tail, head - Capacity)`。**语义**：logical oldest boundary（逻辑最旧有效边界）。当 `head - tail ≤ Capacity`（队列未超容量）时 `logicalDropBoundary = tail`（无 drop，Consumer 从 tail 读）；当 `head - tail > Capacity`（队列超容量，Drop-Oldest 已生效）时 `logicalDropBoundary = head - Capacity`（最旧 `Capacity` 个 slot 有效，更旧的已被覆写 drop）。**logicalDropBoundary 是派生量非存储变量**——无"Consumer 持有 stale logicalDropBoundary"问题（每次 tryPop 重新计算），无 stale-boundary race（不依赖 Producer 单独宣布的 discard cursor，避免 v1.2 publishedTail 的 R14 缺陷）。
- **INV7a（pendingDropped 瞬时派生量，v1.6 Design-R17 新增，v1.7 替代旧 INV7）**：`pendingDropped = logicalDropBoundary - tail = max(0, head - tail - Capacity)`。**语义**：当前尚未被 Consumer 对齐的 dropped prefix 长度（瞬时派生量）。当 `head - tail ≤ Capacity` 时 `pendingDropped = 0`（无 drop）；当 `head - tail > Capacity` 时 `pendingDropped = head - tail - Capacity`。**随 Consumer 推进 tail 而归零**（对齐后 `logicalDropBoundary = tail`，`pendingDropped = 0`）。**这是 correctness invariant**，参与 correctness 证明（Consumer 通过 logicalDropBoundary 对齐步骤保证 `pendingDropped` 最终归零）。
- **INV7b（cumulativeDropCount 独立统计量，v1.6 Design-R17 新增，v1.7 替代旧 INV7 累计语义 + 明确为 observational metric）**：`cumulativeDropCount` 为 `std::atomic<uint64_t>`，由 Producer 在 `tryPushDropOldest` 中 `if (h - tail.load(acquire) >= Capacity) cumulativeDropCount.fetch_add(1, relaxed)` 维护。**语义**：自系统启动以来 Producer 观察到队列物理满或超容量而丢弃的 slot 累计数。**单调递增不回退**。**降级为 observational metric，非 correctness invariant**——其值依赖 Producer 读 `tail` 的时机（acquire 读可能看到稍旧的 Consumer 进度），不参与 correctness 证明。用途为监控告警（drop rate 异常）/ 容量调优（SPSC_DATA capacity 是否足够）/ DFX 指标上报。与 `pendingDropped` 关系为 `cumulativeDropCount ≥ pendingDropped` 但两者不必相等。
- **区间分解（v1.6 Design-R17-B 新增，v1.7 替代旧 INV8 代数恒等式）**：`[0, head) = [0, logicalDropBoundary) ∪ [logicalDropBoundary, tail) ∪ [tail, head)`，其中 `[0, logicalDropBoundary)` = logically expired（已被 Drop-Oldest 逻辑丢弃的 slot，Consumer 不应再读）、`[logicalDropBoundary, tail)` = impossible/empty（因 `logicalDropBoundary ≥ tail`，不存在既未被 drop 又已被 Consumer 越过的 slot）、`[tail, head)` = current logical window（Consumer 可读的有效窗口，长度 `head - tail`，其中 `[tail, logicalDropBoundary)` 已 drop（`pendingDropped` 个），`[logicalDropBoundary, head)` 为有效未 drop（`head - logicalDropBoundary` 个，≤ Capacity））。**性质**：(1) `tail ≤ logicalDropBoundary ≤ head`；(2) `head - logicalDropBoundary ≤ Capacity`（逻辑窗口 bounded）；(3) `[logicalDropBoundary, tail)` 为空区间；(4) 区间不重叠且覆盖 `[0, head)`。**这是 correctness invariant**，明确描述 `[0, head)` 的逻辑划分，不声称代数恒等式等于累计计数器 `cumulativeDropCount`（`cumulativeDropCount` 作为独立 observational metric，不进入区间分解）。
- **~~旧 INV7（v1.5 Design-R16，v1.7 彻底废弃）~~**：~~`droppedCount = effectiveTail - tail = max(0, head - tail - Capacity)`~~（v1.6 识别概念混淆：`effectiveTail - tail` 是瞬时派生量但 v1.5 同时用它代表累计 counter，两者不等价。v1.7 彻底从 Contract 中删除，拆为 INV7a 瞬时派生量 + INV7b 独立累计统计量。旧定义仅在 Amendment v1.5/v1.6 变更记录中保留作为历史记录）。
- **~~旧 INV8（v1.5 Design-R16，v1.7 彻底废弃）~~**：~~`head = droppedCount + tail + (head - effectiveTail)`~~（v1.6 识别为代数恒等式而非运行时累计计数器等式。v1.7 彻底从 Contract 中删除，替换为区间分解。旧定义仅在 Amendment v1.5/v1.6 变更记录中保留作为历史记录）。
...
**Consumer tryPop（A++：seqlock double-read validation — R15 核心修复 + R16 logicalDropBoundary 对齐，v1.7 命名统一）**：
```
Consumer:
  ct = tail.load(memory_order_relaxed)                 // Consumer-owned, relaxed
  h  = head.load(memory_order_acquire)                 // 读 Producer-owned head（acquire）
  // ★★★ R16 核心修复：logicalDropBoundary 对齐（logical oldest boundary）★★★
  logicalDropBoundary = max(ct, h - Capacity)          // ★ 派生量计算（INV6），无竞态（ct/h 均已原子读）
  if (ct < logicalDropBoundary) {
      // Consumer 落后于 effective oldest boundary：[ct, logicalDropBoundary) 区间的 slot 已被 Drop-Oldest 丢弃
      // 快速推进 tail 到 logicalDropBoundary，跳过已被 drop 的 slot（避免逐个 seq 校验失败循环）
      tail.store(logicalDropBoundary, memory_order_release)  // 推进 Consumer-owned tail
      ct = logicalDropBoundary                               // 对齐到 logical oldest boundary
  }
...
> **R16 核心修复点**：v1.4 的 Consumer tryPop 从 `ct = tail` 直接开始 seqlock double-read，当 Consumer 持有 stale ct（如 ct=100，但 head=1256, Capacity=256, logicalDropBoundary=1000）时，Consumer 会从 slot 100 开始逐个 seq 校验失败、推进 tail、continue 循环 900 次才到达有效 slot——**正确但低效**，且 v1.4 的 INV2 `head - tail ≤ Capacity` 在此状态下不成立（1256 - 100 = 1156 > 256）。v1.5 的 Consumer tryPop 在进入 seqlock double-read 前**先计算 logicalDropBoundary = max(ct, h - Capacity) 并对齐**：若 ct < logicalDropBoundary，直接推进 tail 到 logicalDropBoundary（一次性跳过所有已被 Drop-Oldest 丢弃的 slot），然后从 logicalDropBoundary 开始 seqlock double-read。**这既是效率优化（避免逐个循环），更是 invariant 正确性补充**：对齐后 `ct ≥ logicalDropBoundary`，`head - ct ≤ head - logicalDropBoundary ≤ Capacity`（INV2 成立）。**seqlock double-read validation 逻辑不变**（R15 成果保留），logicalDropBoundary 对齐只在进入 double-read 前增加一步"跳过已 drop 的 slot"。
...
> 2. **不变量（v1.7 Contract Consistency Cleanup：废弃 INV2-OLD，统一使用 logicalDropBoundary + INV7a/INV7b + 区间分解，彻底删除旧 INV7/INV8）**：`tail ≤ head`（INV1）；~~`head - tail ≤ Capacity`（INV2-OLD，v1.4 已废弃，Drop-Oldest 模式下不成立）~~；`head - logicalDropBoundary ≤ Capacity`（INV2，v1.5 重新定义，`logicalDropBoundary = max(tail, head - Capacity)`，逻辑占用 bounded 恒成立）；`head`/`tail` 单调递增（INV3，ABA 不发生）；`buffer[i].seq == k×Capacity + i + 1`（INV4，slot epoch invariant）；`buffer[i].payload.words[k]` 均为 `std::atomic<uint64_t>` 且 `is_lock_free()` = true（INV5，word-atomic invariant，A++ 新增）；`logicalDropBoundary = max(tail, head - Capacity)`（INV6，logical oldest boundary 派生量，v1.5 Design-R16 新增，v1.7 命名统一）；`pendingDropped = logicalDropBoundary - tail = max(0, head - tail - Capacity)`（INV7a，瞬时派生量，随 Consumer 推进 tail 而归零，v1.6 Design-R17 新增，v1.7 替代旧 INV7）；`cumulativeDropCount` 为 `std::atomic<uint64_t>` 独立累计统计量，单调递增不回退，**降级为 observational metric 非 correctness invariant**（INV7b，v1.6 Design-R17 新增，v1.7 替代旧 INV7 累计语义）；区间分解 `[0, head) = [0, logicalDropBoundary) ∪ [logicalDropBoundary, tail) ∪ [tail, head)`（logically expired / impossible-empty / current logical window，v1.6 Design-R17-B 新增，v1.7 替代旧 INV8 代数恒等式，明确 `tail ≤ logicalDropBoundary ≤ head` + `head - logicalDropBoundary ≤ Capacity`）。**旧 INV7 `droppedCount = effectiveTail - tail` 和旧 INV8 `head = droppedCount + tail + (head - effectiveTail)` 已从 Contract 中彻底删除（v1.7），仅在 Amendment v1.5/v1.6 变更记录中保留作为历史记录。**
...
> 5. **Consumer tryPop（v1.5 Design-R16 logicalDropBoundary 对齐 + R15 seqlock double-read，v1.7 命名统一）**：`ct = tail.load(relaxed)` → `h = head.load(acquire)` → `logicalDropBoundary = max(ct, h - Capacity)`（INV6 派生量计算）→ 若 `ct < logicalDropBoundary` 推进 `tail` 到 `logicalDropBoundary`（R16 对齐，跳过已被 Drop-Oldest 丢弃的 slot）→ `s1 = seq.load(acquire)` → epoch 校验 `s1 == ct+1` → `item = payload.load(relaxed)`（逐 word atomic）→ `s2 = seq.load(acquire)` → 覆写校验 `s1 == s2`，两校验都通过才接受 item，否则丢弃跳过。
...
> 14. **容量语义（v1.5 Design-R16 修正，v1.7 命名统一）**：`SPSC_DATA_CAPACITY = 256`，**逻辑占用 `head - logicalDropBoundary ≤ 256`**（INV2，恒成立）；物理占用 `head - tail` 在 Drop-Oldest 模式下**可超过 256**（tail 不跟随 head 推进，Producer 连续写入后 `head - tail` 可远超 Capacity，但 `head - logicalDropBoundary ≤ Capacity` 恒成立）；编译期 `static_assert`，2 的幂次。
...
**与 v1.1/v1.2 的关系链**：Design-R7（v1.1）识别 "Producer 不得移动 Consumer-owned tail" → Design-R11（v1.2）用 `publishedTail` 部分解决但留下 R14 linearizability → Design-R14（v1.3）用 seqlock slot 闭环 linearizability 但留下 R15 lifetime race → **Design-R15（v1.4）用 word-atomic payload + seqlock double-read 彻底解决 data race** → **Design-R16（v1.5）重新定义 Queue Invariant（logicalDropBoundary 派生量，v1.5 原名 effectiveTail，v1.7 全文统一重命名）解决 Capacity Invariant Contradiction** → **Design-R17（v1.6）概念分离 logicalDropBoundary/pendingDropped/cumulativeDropCount + 区间分解替代旧 INV7/INV8** → **Design-R17-CONSISTENCY（v1.7）彻底删除旧 INV7/INV8 + 全文统一命名 + cumulativeDropCount 明确为 observational metric** → **Design-R18（v1.6）完整 Push/Pop 线性化证明 P1～P8** → **Design-R18-FOUR-ELEMENTS + Design-R18-THEOREM（v1.7）P1-P8 明确四要素 + Epoch Validity Linearization Theorem**。

---

**Design-R16 修复：Drop-Oldest Queue Logical Boundary / Capacity Invariant Contradiction（v1.5 新增，v1.7 命名统一 effectiveTail → logicalDropBoundary）**：
...
>
> **回答**：**`logicalDropBoundary = max(tail, head - Capacity)`**（v1.5 原名 `effectiveTail`，v1.7 全文统一重命名以强调语义）。这是一个**派生量**（非直接存储的原子变量），由 Producer-owned `head` 和 Consumer-owned `tail` 的数学关系定义。
>
> **为什么 logicalDropBoundary 是正确的 logical oldest boundary**：
> 1. **不是 Producer 单独宣布的**：避免 v1.2 `publishedTail` 的 R14 stale-boundary race（Producer 宣布 discard 后 Consumer 持有 stale `publishedTail` 仍读被 drop 的 slot）。logicalDropBoundary 由 `head` 和 `tail` 共同决定，Producer 推进 `head` 时 logicalDropBoundary 自动跟进（`head - Capacity` 增大）。
> 2. **不是 Consumer 单独决定的**：避免 Consumer 暂停时 logical oldest boundary 不前进（Consumer 暂停时 `tail` 不变，但 `head` 继续推进，`head - Capacity` 增大，logicalDropBoundary = `head - Capacity` 跟进）。
> 3. **无 stale-boundary race**：logicalDropBoundary 是**每次 tryPop 重新计算的派生量**，不是存储的变量。Consumer 每次 tryPop 读 `head`（acquire）得到**当前最新** head，重新计算 logicalDropBoundary。不存在"Consumer 持有 stale logicalDropBoundary"的竞态窗口——logicalDropBoundary 是局部变量，用完即弃。
> 4. **数学上恒满足 `head - logicalDropBoundary ≤ Capacity`**：`logicalDropBoundary = max(tail, head - Capacity) ≥ head - Capacity`，故 `head - logicalDropBoundary ≤ Capacity`。这是**恒等式**，不依赖任何时序假设。
> 5. **与 seqlock double-read 正交**：logicalDropBoundary 告诉 Consumer "应该从哪个 ct 开始读"（logical oldest boundary），seqlock double-read 告诉 Consumer "这个 slot 是否仍然有效"（slot-level validity）。两者职责分离，不冲突。

**logicalDropBoundary 无竞态证明**：

- logicalDropBoundary = `max(tail.load(relaxed), head.load(acquire) - Capacity)`，由两个原子读的结果计算。
...
- 两个原子读各自返回确定值，`max` 运算是纯函数，logicalDropBoundary 是确定值。**不存在"logicalDropBoundary 被并发修改"的竞态**——它是局部计算结果，非共享变量。✓
- **与 v1.2 publishedTail 的关键区别**：publishedTail 是**存储的共享原子变量**，Producer 写、Consumer 读，存在"Consumer 持有 stale publishedTail 局部副本"的竞态窗口。logicalDropBoundary 是**派生的局部变量**，每次 tryPop 重新计算，不存在 stale 副本问题。✓

**Consumer-pause interleaving 补充分析（R16，大G项目经理要求 — 五种状态完备）**：

> v1.4 的 J1～J8 interleaving 分析的是 **slot-level** Consumer-pause（Consumer 在 seqlock double-read 的某个点暂停）。R16 补充 **queue-level** Consumer-pause 分析：Consumer 暂停期间 Producer 持续写入，`head` 推进远超 `tail`，logicalDropBoundary 如何保持逻辑一致。
...
|------|------|--------|---------|--------|--------|
| **Q1 Full（队列满，无 Consumer 消费）** | `head - tail == Capacity`，Consumer 暂停 | `logicalDropBoundary = max(tail, h - Capacity) = tail`（因 `h - tail == Capacity` → `h - Capacity == tail`） | Producer 写入 m 次：`head = h + m`，`tail` 不变，`logicalDropBoundary = max(tail, h + m - Capacity) = h + m - Capacity > tail`（Drop-Oldest 生效，最旧 m 个 slot 被 drop） | Consumer 恢复，重新读 `head = h + m`（acquire），计算 `logicalDropBoundary = h + m - Capacity`，`ct = tail < logicalDropBoundary` → 推进 `tail` 到 `logicalDropBoundary`（R16 对齐，跳过 m 个已 drop slot）→ 从 `logicalDropBoundary` 开始 seqlock double-read | ✓ **R16 对齐一次性跳过 m 个 drop slot**，避免逐个 seq 校验失败循环；INV2 `head - logicalDropBoundary = Capacity` 恒成立 |
| **Q2 Non-Full（队列未满，Consumer 暂停）** | `head - tail < Capacity`，Consumer 暂停 | `logicalDropBoundary = tail`（因 `h - Capacity < tail`） | Producer 写入 m 次，若 `h + m - tail ≤ Capacity`（仍未满）：`logicalDropBoundary = tail`（无 drop）；若 `h + m - tail > Capacity`（超容量）：`logicalDropBoundary = h + m - Capacity > tail`（Drop-Oldest 生效） | Consumer 恢复，重新计算 logicalDropBoundary；若 `ct < logicalDropBoundary` 则对齐（R16），否则正常 seqlock double-read | ✓ 未满时 logicalDropBoundary = tail，无 drop；超容量时 R16 对齐 |
| **Q3 Consumer Paused（Consumer 长期暂停，Producer 持续写入）** | Consumer 暂停 Δt → ∞，Producer 持续写入 | 任意状态 | `head` 持续推进，`tail` 不变，`head - tail → ∞`，但 `head - logicalDropBoundary = head - (head - Capacity) = Capacity`（INV2 恒成立） | Consumer 恢复，读 `head`（acquire），计算 `logicalDropBoundary = head - Capacity`，对齐跳过所有已 drop slot，从最旧有效 slot 开始读 | ✓ **INV2 在 Consumer 任意暂停期间恒成立**（logicalDropBoundary 跟随 head 推进）；恢复后 R16 对齐 |
| **Q4 Consumer Validating（Consumer 在 seqlock double-read 中暂停）** | Consumer 已通过 logicalDropBoundary 对齐，正在 seqlock double-read（s1 已读 / payload 读中 / s2 未读），暂停 | `ct ≥ logicalDropBoundary`（已对齐） | Producer 覆写当前 slot（head 推进），`logicalDropBoundary` 可能增大，但 Consumer 已持有 `ct`（对齐后的值），不重新计算 logicalDropBoundary | Consumer 恢复，继续 seqlock double-read：若 payload 读期间被覆写，`s1 != s2` → 丢弃（J7/J8，R15 修复）；Consumer 下次 tryPop 重新计算 logicalDropBoundary | ✓ **slot-level 暂停由 R15 J7/J8 覆盖**（word-atomic + double-seq 校验）；queue-level logicalDropBoundary 在下次 tryPop 重新计算 |
| **Q5 Consumer After Payload Copy（Consumer 已 copy item 到栈，尚未推进 tail）** | Consumer 已 `s1 == s2 == ct+1`，`item = payload.load()` 完成，尚未 `tail.store(ct+1)`，暂停 | item 在 Consumer 栈上 | Producer 覆写该 slot（下一轮），`head` 推进，`logicalDropBoundary` 可能增大 | Consumer 恢复，`tail.store(ct+1, release)`（推进 tail），return item（栈上副本，不受 Producer 覆写影响）；下次 tryPop 重新计算 logicalDropBoundary | ✓ Consumer 持有栈上副本，Producer 覆写不影响；tail 推进后下次 logicalDropBoundary 可能 = `tail`（若未超容量）或 `head - Capacity`（若超容量） |

> **五种状态完备性**：Consumer 在任意时刻处于以下状态之一：(a) 未开始 tryPop（Q1/Q2/Q3 的起点）、(b) logicalDropBoundary 对齐中（Q1/Q2/Q3 恢复后）、(c) seqlock double-read 中（Q4）、(d) 已 copy item 待推进 tail（Q5）。Producer 在 Consumer 任意状态下均可推进 head（非阻塞）。**五种状态穷举全部 Consumer-pause × Producer-write interleaving**，INV2 `head - logicalDropBoundary ≤ Capacity` 在所有状态下恒成立。✓

**区间分解证明 + cumulativeDropCount observational metric 声明（v1.7 重写，替代 v1.5 统计恒等式 + droppedOldestCount 统计修正）**：

> **v1.7 Design-R17-OBSERVATIONAL 修正**：v1.5 的统计恒等式 `head = droppedCount + tail + (head - effectiveTail)` 是**代数恒等式**（纯代数展开），没有证明运行时累计计数器 `droppedOldestCount` 等于 `effectiveTail - tail`。v1.6 识别此问题并引入区间分解 + cumulativeDropCount 降级为 observational metric。v1.7 彻底重写本节，删除旧统计恒等式表述，明确 correctness 由区间分解保证，cumulativeDropCount 不参与 correctness 证明。

> **定理（区间分解，v1.7 替代 v1.5 统计恒等式）**：在任意时刻，`[0, head)` 可分解为三个不相交区间：
> - `[0, logicalDropBoundary)` = logically expired（已被 Drop-Oldest 逻辑丢弃的 slot，共 `logicalDropBoundary` 个）
> - `[logicalDropBoundary, tail)` = impossible/empty（因 `logicalDropBoundary ≥ tail`，此区间为空）
> - `[tail, head)` = current logical window（Consumer 可读的有效窗口，共 `head - tail` 个，其中 `[tail, logicalDropBoundary)` 已 drop（`pendingDropped` 个，INV7a），`[logicalDropBoundary, head)` 为有效未 drop（`head - logicalDropBoundary` 个，≤ Capacity，INV2））
>
> **性质证明**：
> 1. `tail ≤ logicalDropBoundary ≤ head`：`logicalDropBoundary = max(tail, head - Capacity) ≥ tail`（max 下界）；`logicalDropBoundary ≤ max(head, head) = head`（因 `tail ≤ head` 且 `head - Capacity ≤ head`）✓
> 2. `head - logicalDropBoundary ≤ Capacity`（逻辑窗口 bounded）：`logicalDropBoundary ≥ head - Capacity`，故 `head - logicalDropBoundary ≤ Capacity` ✓
...
>
> **correctness 保证**：correctness 由 seqlock double-read validation（R15）+ logicalDropBoundary 对齐（R16）+ 区间分解（R17-B）保证。Consumer 通过 logicalDropBoundary 对齐步骤跳过 `[0, logicalDropBoundary)` 的 logically expired slot，通过 seqlock double-read 校验跳过 `[tail, logicalDropBoundary)` 的已 drop slot，仅接受 `[logicalDropBoundary, head)` 的有效未 drop slot。**不依赖 `cumulativeDropCount` 的精确值**。

> **cumulativeDropCount observational metric 声明（v1.7 明确）**：
> - `cumulativeDropCount`（即 `droppedOldestCount` 运行时值）为 `std::atomic<uint64_t>`，由 Producer 在 `tryPushDropOldest` 中 `if (h - tail.load(acquire) >= Capacity) cumulativeDropCount.fetch_add(1, relaxed)` 维护。
> - **不作为 correctness invariant**，降级为 **observational metric**。其值依赖 Producer 读 `tail` 的时机（`tail.load(acquire)` 可能看到稍旧的 Consumer 进度），不是严格可证明的不变量。例如：Consumer 已推进 `tail` 但 Producer 的 acquire 读尚未看到，Producer 可能多计一次 drop。
> - **用途**：监控告警（drop rate 异常）、容量调优（SPSC_DATA capacity 是否足够）、DFX 指标上报。
> - **与 correctness 的关系**：即使 `cumulativeDropCount` 多计或少计，Consumer 仍通过 seqlock double-read + logicalDropBoundary 对齐正确跳过被覆写 slot。**correctness 不依赖 `cumulativeDropCount` 的精确值**。
> - **可观测性**：`cumulativeDropCount` 作为 `std::atomic<uint64_t>` 可被监控线程读取（relaxed），用于 DFX 指标上报。
> - **与 `pendingDropped` 的关系**：`cumulativeDropCount ≥ pendingDropped`（累计 ≥ 瞬时），但两者不必相等。`cumulativeDropCount = pendingDropped` 当且仅当 Consumer 从未对齐过（tail 从未推进到 logicalDropBoundary）。Consumer 对齐后 `pendingDropped = 0`，但 `cumulativeDropCount` 保留历史累计值。
...
- **logicalDropBoundary 可见性**：logicalDropBoundary = `max(tail, head - Capacity)`，`head.load(acquire)` 保证 Consumer 看到最新 head，`tail.load(relaxed)` 返回 Consumer-owned tail（单线程读自己的写，无需同步）。logicalDropBoundary 的可见性等价于 head 的可见性。✓
...
- **定义**：
  ...
  - logicalDropBoundary 对齐的 linearization point = Consumer 执行 `tail.store(logicalDropBoundary, release)` 的瞬间（`L_align`，若触发）。
- **proof**：
  ...
  - `L_align` 线性化为"Consumer 跳过 [ct, logicalDropBoundary) 区间的已 drop slot"，在 Producer 已 drop 这些 slot 之后（因 `logicalDropBoundary = head - Capacity`，Producer 推进 head 到 `logicalDropBoundary + Capacity` 之后这些 slot 才被 drop）。✓
...
> | linearization | 并发历史层 | 并发→顺序映射 | `L2`（tryPop）/ `Lp`（tryPush）/ `L_align`（logicalDropBoundary 对齐） |
...
- 不变量审查：`tail ≤ head`（INV1）+ `head - logicalDropBoundary ≤ Capacity`（INV2，v1.5 Design-R16，v1.7 命名统一）+ `logicalDropBoundary = max(tail, head - Capacity)`（INV6）+ `pendingDropped = logicalDropBoundary - tail`（INV7a，瞬时派生量，v1.6 Design-R17）+ `cumulativeDropCount` 为 `std::atomic<uint64_t>` 独立累计统计量降级为 observational metric（INV7b，v1.6 Design-R17）+ 区间分解 `[0, head) = [0, logicalDropBoundary) ∪ [logicalDropBoundary, tail) ∪ [tail, head)`（v1.6 Design-R17-B，替代旧 INV8）+ slot epoch invariant（INV4）+ word-atomic invariant（INV5）（HARD CONTRACT，源码审查 + 运行时断言）。**旧 INV7 `droppedCount = effectiveTail - tail` 和旧 INV8 `head = droppedCount + tail + (head - effectiveTail)` 已从审查清单中彻底删除（v1.7），仅在 Amendment v1.5/v1.6 变更记录中保留作为历史记录。**
...
- **R16 logicalDropBoundary 对齐测试（TEST-R16-LOGICAL-DROP-BOUNDARY）**：Consumer 长期暂停（sleep 100ms）期间 Producer 高频写入 1000+ 事件（head 远超 tail + Capacity），Consumer 恢复后验证：(1) `logicalDropBoundary = max(tail, head - Capacity)` 正确计算；(2) Consumer 一次性对齐到 logicalDropBoundary（非逐个 seq 校验失败循环）；(3) INV2 `head - logicalDropBoundary ≤ Capacity` 恒成立；(4) 区间分解 `tail ≤ logicalDropBoundary ≤ head` + `[logicalDropBoundary, tail)` 为空成立（HARD CONTRACT，CI nightly 测试）。
- **R16 INV2 恒成立运行时断言**：每次 tryPop 后断言 `head - logicalDropBoundary <= Capacity`（运行时断言，DEBUG build）。
- **R16 五种 Consumer-pause 状态测试**：Q1 Full / Q2 Non-Full / Q3 Consumer Paused / Q4 Consumer Validating / Q5 Consumer After Payload Copy 五种状态各设计专门测试用例，验证 INV2 恒成立 + logicalDropBoundary 对齐正确（HARD CONTRACT，CI nightly 测试）。
...
**Design-R17 修复：effectiveTail / Drop-Oldest logical semantics + accounting consistency（v1.6 新增，v1.7 Contract Consistency Cleanup 彻底闭环）**：

> **问题回顾**：v1.5 定义 INV7 `droppedCount = effectiveTail - tail`（旧名 effectiveTail 即 v1.7 的 logicalDropBoundary），同时 Producer 实现是 `if (h - tail.load(acquire) >= Capacity) droppedOldestCount.fetch_add(1)`。存在两个不同概念被混为一个名字，导致统计语义不一致。**v1.7 进一步彻底从 Contract / 不变量列表 / Verification Matrix 中删除旧 INV7/INV8 定义**（v1.6 仅在旧定义旁边加修正注释，v1.7 彻底清除），全文统一使用 logicalDropBoundary / pendingDropped / cumulativeDropCount 三量分离 + 区间分解。
...
Consumer 消费 100 个 (tryPop × 100, 含 logicalDropBoundary 对齐):
...
[0, head)  ← Producer 已写入的全部 slot（共 head 个）
  ...
  │     Consumer 不应再读这些 slot（logicalDropBoundary 对齐步骤跳过）
  │
...
- **与 correctness 的关系**：correctness 由 seqlock double-read validation（R15）+ logicalDropBoundary 对齐（R16）+ 区间分解（R17-B）保证，**不依赖 `cumulativeDropCount` 的精确值**。即使 `cumulativeDropCount` 多计或少计，Consumer 仍通过 seqlock double-read 正确跳过被覆写 slot。
- **可观测性**：`cumulativeDropCount` 作为 `std::atomic<uint64_t>` 可被监控线程读取（relaxed），用于 DFX 指标上报。

**R17 可验证 invariant（v1.7 更新：彻底删除旧 INV7/INV8 + 全文统一命名）**：
...
- 源码审查：**旧 INV7 `droppedCount = effectiveTail - tail` 已从 Contract / 不变量列表 / Verification Matrix 中彻底删除**（v1.7 Design-R17-CONSISTENCY），仅在 Amendment v1.5/v1.6 变更记录中保留作为历史记录（HARD CONTRACT，源码审查 + 文档审查）。
- 源码审查：**旧 INV8 `head = droppedCount + tail + (head - effectiveTail)` 已从 Contract / 不变量列表 / Verification Matrix 中彻底删除**（v1.7 Design-R17-CONSISTENCY），替换为区间分解，仅在 Amendment v1.5/v1.6 变更记录中保留作为历史记录（HARD CONTRACT，源码审查 + 文档审查）。
- 源码审查：**全文正文统一使用 logicalDropBoundary（替代 effectiveTail）/ pendingDropped（替代 droppedCount 瞬时量语义）/ cumulativeDropCount（替代 droppedOldestCount 累计量语义）**（v1.7 Design-R17-NAMING），旧名仅在 Amendment v1.5/v1.6 变更记录 + R17/R18 修复子节的历史回顾中保留（HARD CONTRACT，文档审查）。
- 源码审查：**`cumulativeDropCount` 明确为 observational metric，不参与 correctness 证明**（v1.7 Design-R17-OBSERVATIONAL），Verification Matrix 中不要求 `cumulativeDropCount` 满足任何 correctness 条件（HARD CONTRACT，源码审查 + 文档审查）。
...
**Design-R18 修复：Drop-Oldest linearization under derived logicalDropBoundary + concurrent head advancement（v1.6 新增，v1.7 P1-P8 明确四要素 + Epoch Validity Linearization Theorem）**：

> **问题回顾**：v1.5 声称"effectiveTail（v1.7 重命名为 logicalDropBoundary）是派生量所以不存在 stale-boundary race"，但 **no stale variable race ≠ no semantic linearization race**。v1.5 仅给出 logicalDropBoundary 无竞态证明（派生量非存储变量），未给出完整 Push/Pop 两操作线性化证明。同时 v1.5 混合了 Drop 发生时间的三个定义。**v1.7 进一步要求 P1-P8 每种情况明确四要素**（Producer LP / Consumer LP / Drop LP（语义发生点）/ Consumer 最终接受/拒绝原因）**+ 增加 Theorem (Epoch Validity Linearization)** 形式化证明"已被 Producer Push LP 之前失效的 epoch 不得被 Consumer 在其 Pop LP 之后返回"。
...
| 定义 B（Consumer-observed drop via logicalDropBoundary） | Consumer 下一次观察 head 时 `logicalDropBoundary > tail` 才认为被丢弃 | Consumer 发现 drop 的机制 B（非 drop 发生定义） |
...
2. **定义 B 和 C 是 Consumer-side 的观察**，是定义 A 的**后果**（Consumer 迟早会观察到 drop，但观察时机不定义 drop 发生时机）。机制 B（logicalDropBoundary 对齐）和机制 C（seqlock double-read 校验）是 Consumer 发现已发生 drop 的两种途径，不定义 drop 何时发生。
3. **定义 A 与 `logicalDropBoundary = max(tail, head - Capacity)` 一致**：当 `head` 推进到 `tail + Capacity` 之后，`logicalDropBoundary = head - Capacity > tail`，position `[tail, head - Capacity)` 被 drop（定义 A）。Consumer 通过机制 B（logicalDropBoundary 对齐）或机制 C（seq 校验）发现这些 drop。
...
> - **logicalDropBoundary 对齐 LP** = Consumer 执行 `tail.store(logicalDropBoundary, memory_order_release)` 的瞬间（记为 `L_align`，若触发）。这是 Consumer 跳过 dropped prefix 的瞬间。
> - **Drop LP / 语义发生点（v1.7 Design-R18-FOUR-ELEMENTS 新增）** = Producer 执行 `head.store(h+1, memory_order_release)` 使得 `head - Capacity` 推进到 position `h+1-Capacity` 的瞬间，此时 position `h+1-Capacity` 立即逻辑失效（定义 A：Producer-side logical drop）。即 Drop LP for position `p` = Producer 推进 `head` 到 `p + Capacity` 的 `head.store(release)` 瞬间。

设 Producer 执行 `tryPushDropOldest`（写 slot `i = h % Capacity`，新 epoch `e_new = h+1`），Consumer 执行 `tryPop`（读 slot `j = ct % Capacity`，期望 epoch `e_old = ct+1`）。按 Consumer 操作相对于 Producer 操作的时序，穷举所有 interleaving（含 arbitrary consumer pause）。**v1.7 Design-R18-FOUR-ELEMENTS：每种情况明确四要素（Producer LP / Consumer LP / Drop LP（语义发生点）/ Consumer 最终接受/拒绝原因）**：

| Case | 时序描述 | Producer LP | Consumer LP | Drop LP / 语义发生点 | Consumer 行为 | Consumer 最终接受/拒绝原因 | 正确性结论 |
|------|---------|------------|------------|---------------------|------------|---------------------------|-----------|
| **P1** | Consumer Pop 完全在 Producer Push 之前（`Lc < Lp`） | `Lp`（seq.store 在 Consumer 之后） | `Lc`（s2 读在 Producer 之前） | 无 Drop 发生（Producer 尚未推进 head，position j 未失效） | Consumer 读 `head = h`（Producer 尚未推进），`ct = tail`，在 `[tail, h)` 内 seqlock double-read slot j。Producer 尚未写 slot i，Consumer 读到的是之前的 item。`s1 == s2 == ct+1`（无覆写）→ 返回旧 item | **接受**：epoch 校验 `s1 == ct+1` ✓ + 覆写校验 `s1 == s2` ✓（Producer 尚未覆写） | ✓ **`Lc < Lp` ⇒ old item may be returned legitimately**（线性化为 Consumer Pop < Producer Push） |
| **P2** | Producer Push 完全在 Consumer Pop 之前（`Lp < Lc`） | `Lp`（seq.store 在 Consumer 之前） | `Lc`（s2 读在 Producer 之后） | Drop LP = Producer 推进 head 到 `tail + Capacity` 的瞬间（若此次 Push 使 head 超过 tail + Capacity，则 position `head - Capacity` 失效） | Consumer 读 `head = h+1`（acquire，看到 Producer 写入），`ct = tail`。若 Consumer 读 slot i（j == i）：`s1 = h+1`，若 `ct == h` 则 `s1 == ct+1` ✓，`payload.load`，`s2 = h+1`（Producer 已写完不再覆写），`s1 == s2 == ct+1` → 返回新 item | **接受**：epoch 校验 `s1 == ct+1 == h+1` ✓（Consumer 期望的正是新 slot）+ 覆写校验 `s1 == s2` ✓ | ✓ **`Lp < Lc` ⇒ new item returned**（线性化为 Producer Push < Consumer Pop） |
| **P3** | Consumer 读 head，Producer pushes | `Lp`（Producer 在 Consumer head.load 之后 seq.store） | `Lc`（Consumer s2 读） | Drop LP = Producer 推进 head 到 `h_c + 1` 的瞬间（若 `h_c + 1 > tail + Capacity` 则 position `h_c + 1 - Capacity` 失效，但 Consumer 读的是 slot j < h_c，不受影响） | Consumer `head.load(acquire)` → `h_c = h`（Producer 推进前），`ct = tail`，计算 `logicalDropBoundary = max(tail, h - Capacity)`（基于旧 h_c）。Producer 写 slot h（`i = h % Capacity`），`head = h+1`。Consumer 在 `[tail, h_c)` 内读 slot j < h_c。Producer 写的是 slot h，j ≠ h，不冲突。`s1 == s2 == j+1` → 返回 slot j 的 item | **接受**：epoch 校验 `s1 == j+1 == ct+1` ✓ + 覆写校验 `s1 == s2` ✓（Producer 写的是 slot h_c ≠ j，不覆写 slot j） | ✓ Consumer LP 基于 `h_c`，Producer LP 在 `h_c` 之后。Consumer 读 slot j（j < h_c），Producer 写 slot h_c，不冲突。**线性化为 Consumer Pop (slot j) < Producer Push (slot h_c)** |
| **P4** | Consumer 计算 logicalDropBoundary，Producer pushes | `Lp`（Producer 在 Consumer logicalDropBoundary 计算之后 seq.store） | `Lc`（Consumer s2 读） | Drop LP = Producer 推进 head 到 `h_c + 1` 的瞬间（position `h_c + 1 - Capacity` 失效，若 `h_c + 1 - Capacity > tail`）；Consumer 的 `L_align` 线性化为跳过 `[tail, logicalDropBoundary)` 的已 drop slot | Consumer `head.load` → `h_c`，计算 `logicalDropBoundary = max(tail, h_c - Capacity)`（基于旧 h_c）。Producer 推进 `head` 到 `h_c + 1`。Consumer 对齐：若 `ct < logicalDropBoundary`，`tail.store(logicalDropBoundary)`，`ct = logicalDropBoundary`（`L_align`）。Consumer 在 `[ct, h_c)` 内 seqlock double-read slot j ∈ [ct, h_c)，j < h_c。Producer 写 slot h_c，j ≠ h_c，不冲突 | **接受**：epoch 校验 `s1 == j+1 == ct+1` ✓ + 覆写校验 `s1 == s2` ✓（Producer 写的是 slot h_c ≠ j）；若 `ct < logicalDropBoundary` 则 `L_align` 跳过 `[tail, logicalDropBoundary)` 的已 drop slot（定义 A：这些 slot 在 Producer 推进 head 到 `logicalDropBoundary + Capacity` 之后已被 drop） | ✓ Consumer 持有基于旧 h_c 的 logicalDropBoundary，Producer 推进 head 不影响 Consumer 在 `[ct, h_c)` 内读。**线性化为 Consumer Pop (slot j < h_c) < Producer Push (slot h_c)** |
| **P5** | Consumer s1，Producer overwrite | `Lp`（Producer 在 Consumer s1 读之后覆写 seq.store） | `Lc`（Consumer s2 读在 Producer 之后） | Drop LP = Producer 覆写 slot j 的 `seq.store(e_new, release)` 瞬间（position j 的旧 epoch 立即失效，定义 A） | Consumer `s1 = seq.load(acquire)` → `s1 = e_old`（旧 epoch）。Producer 覆写同一 slot：`payload.store` + `seq.store(e_new, release)`（`e_new > e_old`）。Consumer `payload.load`（逐 word atomic，无 data race，可能读到部分旧部分新）。Consumer `s2 = seq.load(acquire)` → `s2 = e_new`。`s1 != s2`（`e_old != e_new`）→ 丢弃 | **拒绝**：覆写校验 `s1 != s2`（`e_old != e_new`）✗ → 丢弃跳过（Drop Oldest 生效） | ✓ **`Lp < Lc` ⇒ old item cannot be returned**（Producer 在 Consumer s2 读之前已 `seq.store(e_new)`，`s2 = e_new > s1`，`s1 != s2` → 丢弃）。线性化为 Producer Push < Consumer Pop（Consumer 检测到覆写，丢弃） |
| **P6** | Consumer payload read，Producer overwrite | `Lp`（Producer 在 Consumer payload.load 中途覆写 seq.store） | `Lc`（Consumer s2 读在 Producer 之后） | Drop LP = Producer 覆写 slot j 的 `seq.store(e_new, release)` 瞬间（position j 的旧 epoch 立即失效，定义 A） | Consumer `s1 = seq.load(acquire)` → `s1 = e_old == ct+1`（校验通过）。Consumer `payload.load(relaxed)` 中途暂停。Producer 覆写：`payload.store`（逐 word atomic）+ `seq.store(e_new, release)`。Consumer 恢复，`payload.load` 完成（读到部分旧部分新 word，但每 word atomic 无 data race）。Consumer `s2 = seq.load(acquire)` → `s2 = e_new`。`s1 != s2` → 丢弃（item 撕裂但不使用） | **拒绝**：覆写校验 `s1 != s2`（`e_old != e_new`）✗ → 丢弃跳过（撕裂 item 不使用）；**逐 word atomic 无 data race**（C++20 strict 合规） | ✓ **`Lp < Lc` ⇒ old item cannot be returned**（覆写校验检测到覆写，撕裂 item 被丢弃）。线性化为 Producer Push < Consumer Pop |
| **P7** | Consumer s2，Producer overwrite | `Lp`（Producer 在 Consumer s2 读之后覆写 seq.store） | `Lc`（Consumer s2 读在 Producer 之前） | Drop LP = Producer 覆写 slot j 的 `seq.store(e_new, release)` 瞬间（在 Consumer `Lc` 之后，position j 的旧 epoch 在 Consumer 读取时仍有效） | Consumer `s1 → payload.load → s2 = seq.load(acquire)` → `s2 = e_old`（Producer 覆写前）。`s1 == s2 == ct+1`（校验通过）→ Consumer 接受 item（栈上副本）。Producer 之后覆写 slot（`seq.store(e_new, release)`），不影响 Consumer 栈上副本。Consumer `tail.store(ct+1, release)`，return item | **接受**：epoch 校验 `s1 == ct+1` ✓ + 覆写校验 `s1 == s2 == e_old` ✓（Producer 覆写在 Consumer s2 读之后，s2 读到的是旧 epoch）→ 接受栈上副本 | ✓ **`Lc < Lp` ⇒ old item may be returned legitimately**（Consumer 在 Producer 覆写前完成读取，读到旧 item，合法）。线性化为 Consumer Pop < Producer Push（Consumer 读的是第 ct 轮，Producer 写的是第 ct+Capacity 轮，ct < ct+Capacity） |
| **P8** | Consumer arbitrary pause | 取决于暂停点 | 取决于暂停点 | 取决于暂停点（由 P3～P7 的 Drop LP 覆盖） | Consumer 在任意点暂停 Δt（arbitrary），期间 Producer 执行 m 次 `tryPushDropOldest`。暂停点穷举：(a) `head.load` 前、(b) `head.load` 后 logicalDropBoundary 计算前、(c) logicalDropBoundary 对齐后 s1 前、(d) s1 后 payload.load 前、(e) payload.load 后 s2 前、(f) s2 后 tail.store 前 | 取决于暂停点（由 P3～P7 的接受/拒绝原因覆盖） | ✓ 每种暂停点由 P3～P7 覆盖：(a) 恢复后读 `head = h + m`（最新），重新计算 logicalDropBoundary；(b) Consumer 持有旧 h_c，由 P3 覆盖；(c)(d) logicalDropBoundary 对齐后 Producer 覆写，由 P4/P5 覆盖；(e) payload 读中 Producer 覆写，由 P6 覆盖；(f) s2 后 Producer 覆写，由 P7 覆盖。**所有暂停点存在明确 LP，均合法** |
...
**R18-3-THEOREM：Epoch Validity Linearization Theorem（v1.7 Design-R18-THEOREM 新增）**：

> **Theorem (Epoch Validity Linearization)**：
> 任何已被 Producer Push LP 之前失效的 epoch，不得被 Consumer 在其 Pop LP 之后返回。
> 任何在 Pop LP 之前仍有效的 epoch，可以合法返回。
>
> **形式化**：设 Producer 写 slot i 的 LP 为 `Lp(i)`（即 Producer 执行 `buffer[i].seq.store(i+1, release)` 的瞬间），Consumer 读 slot i 的 Pop LP 为 `Lc(i)`（即 Consumer 执行第二次 `s2 = buffer[i].seq.load(acquire)` 的瞬间）。
>
> **(1) 已失效 epoch 不可返回**：
> 若 epoch(i) 在 `Lp(j)` 时已失效（`j > i` 且 `j - i ≥ Capacity`），则不存在合法线性化使 Consumer 在 `Lc(i) > Lp(j)` 后返回 item(i)。
>
> **证明 (1)**：
> - 设 Producer 写 slot j 的 LP 为 `Lp(j)`（`j > i` 且 `j - i ≥ Capacity`），此时 Producer 执行 `buffer[j % Capacity].seq.store(j+1, release)`。
> - 因 `j - i ≥ Capacity`，slot `i % Capacity` 与 slot `j % Capacity` 是同一物理槽位（`i % Capacity == j % Capacity`）。
> - Producer 在 `Lp(j)` 时覆写了 slot `i % Capacity` 的 seq 为 `j+1`（新 epoch），旧 epoch `i+1` 失效（定义 A：Producer-side logical drop）。
> - Consumer 在 `Lc(i) > Lp(j)` 后执行 `s2 = buffer[i % Capacity].seq.load(acquire)`，因 `Lc(i) > Lp(j)` 且 release/acquire happens-before，Consumer `s2` 看到 `j+1`（新 epoch）。
> - Consumer 的 `s1` 读发生在 `s2` 读之前（seqlock double-read 顺序），`s1` 读到的值 ≤ `j+1`。
>   - 若 `s1 = i+1`（旧 epoch）：`s1 != s2`（`i+1 != j+1` 因 `i != j`）→ 覆写校验失败 → 丢弃（P5/P6 情形）。
>   - 若 `s1 = j+1`（新 epoch）：epoch 校验 `s1 == ct+1` 要求 `ct = j`，但 Consumer 期望的是 slot i（`ct = i`），`s1 = j+1 != i+1 = ct+1` → epoch 校验失败 → 丢弃。
>   - 若 `s1 = k+1`（某个其他 epoch k，`i < k < j`）：同理 `s1 != ct+1`（除非 `k = i`，但 `k > i`）→ epoch 校验失败 → 丢弃。
> - **无论如何，Consumer 不能返回 item(i)**。✓
>
> **(2) 仍有效 epoch 可合法返回**：
> 若 epoch(i) 在 `Lc(i)` 时仍有效（`∀j: Lp(j) < Lc(i) ⇒ j < i + Capacity`），则 Consumer 可合法返回 item(i)。
>
> **证明 (2)**：
> - 条件 `∀j: Lp(j) < Lc(i) ⇒ j < i + Capacity` 意味着：在 Consumer `Lc(i)` 之前，Producer 没有推进 head 到 `i + Capacity` 或更远，即 slot `i % Capacity` 没有被第 `i + Capacity` 轮或更晚的写入覆写。
> - Consumer 的 `s1 = buffer[i % Capacity].seq.load(acquire)` 在 `Lc(i)` 之前（seqlock double-read 顺序），`s1` 读到的是 epoch `i+1`（因 slot i 未被覆写，seq 仍为 `i+1`）。
> - Consumer 的 `s2 = buffer[i % Capacity].seq.load(acquire)` 在 `Lc(i)` 瞬间，因 slot i 在 `Lc(i)` 时仍有效（未被覆写），`s2 = i+1`。
> - `s1 == s2 == i+1 == ct+1`（Consumer 期望 `ct = i`）→ epoch 校验 ✓ + 覆写校验 ✓ → Consumer 接受 item(i)。
> - 线性化为 Consumer Pop (slot i) 在 Producer Push (slot i) 之后、Producer Push (slot i+Capacity) 之前，合法。✓
>
> **Theorem 与 P1～P8 的关系**：Theorem (1) 对应 P5/P6（`Lp < Lc` ⇒ old item cannot be returned），Theorem (2) 对应 P1/P7（`Lc < Lp` ⇒ old item may be returned legitimately）。Theorem 形式化了"已被 Producer Push LP 之前失效的 epoch 不得被 Consumer 在其 Pop LP 之后返回"这一核心安全性质，闭合了 R18 的线性化证明。✓

**R18-4：与 v1.5 logicalDropBoundary 无竞态证明的关系**：

v1.5 的 logicalDropBoundary 无竞态证明（派生量非存储变量，无 stale 副本，v1.5 原名 effectiveTail，v1.7 全文统一重命名）解决的是**变量层竞态**（no stale variable race）。R18 的 P1～P8 线性化证明解决的是**语义层竞态**（no semantic linearization race）。两者层次不同：

| 层次 | 问题 | v1.5 解决 | v1.6 补充 | v1.7 补充 |
|------|------|----------|----------|----------|
| 变量层 | logicalDropBoundary 是否有 stale 副本竞态 | ✓ 派生量非存储变量，无 stale 副本 | （v1.5 已解决） | （v1.5 已解决） |
| 语义层 | Push/Pop 并发是否有线性化顺序 | ✗ 未给出完整证明 | ✓ P1～P8 完整线性化证明 | ✓ P1～P8 每种明确四要素 + Epoch Validity Linearization Theorem |

**no stale variable race ≠ no semantic linearization race**：v1.5 证明了 logicalDropBoundary 无变量竞态，但未证明 Push/Pop 两操作的语义线性化。R18 补充完整语义线性化证明（P1～P8 + Epoch Validity Linearization Theorem），闭合 v1.5 的证明缺口。

**R18 可验证 invariant（v1.7 更新：P1-P8 明确四要素 + Epoch Validity Linearization Theorem）**：
- 文档审查：R18-1 Drop 发生时间统一定义（定义 A 为规范定义，B/C 为 Consumer 发现机制）明确表述（HARD CONTRACT，文档审查）。
- 文档审查：R18-2 P1～P8 八种 interleaving 穷举完备，**每种明确四要素：Producer LP / Consumer LP / Drop LP（语义发生点）/ Consumer 最终接受/拒绝原因**（v1.7 Design-R18-FOUR-ELEMENTS，HARD CONTRACT，文档审查）。
- 文档审查：R18-3 核心结论形式化（`Lp < Lc` ⇒ old item cannot be returned；`Lc < Lp` ⇒ old item may be returned legitimately）明确表述（HARD CONTRACT，文档审查）。
- 文档审查：**R18-3-THEOREM Epoch Validity Linearization Theorem 形式化证明明确**（已失效 epoch 不得被 Consumer 在 Pop LP 后返回；仍有效 epoch 可合法返回；Theorem 与 P1～P8 的关系明确）（v1.7 Design-R18-THEOREM，HARD CONTRACT，文档审查）。
...
- **R18 Drop 定义一致性测试（TEST-R18-DROP-DEFINITION）**：验证定义 A（Producer-side logical drop）与机制 B（logicalDropBoundary 对齐）+ 机制 C（seqlock double-read）一致——Producer 推进 head 到 `tail + Capacity` 后，Consumer 通过机制 B 或 C 发现 drop，不遗漏（HARD CONTRACT，CI nightly 测试）。
- **R18 Epoch Validity Linearization Theorem 测试（TEST-R18-EPOCH-VALIDITY，v1.7 新增）**：验证 Theorem (1) 已失效 epoch 不可返回（Producer 写 slot i 后写 slot i+Capacity 覆写，Consumer 在覆写后 tryPop slot i 不得返回 item(i)）+ Theorem (2) 仍有效 epoch 可合法返回（Consumer 在 Producer 覆写前 tryPop slot i 可返回 item(i)）（HARD CONTRACT，CI nightly 测试）。
...
| Design-R7（Drop Oldest 缺并发证明，v1.1 新增 / v1.2 Design-R11 修正 / v1.3 Design-R14 修正 linearizability / v1.4 Design-R15 修正 Slot Lifetime Race / v1.5 Design-R16 修正 Capacity Invariant / v1.7 命名统一） | 🔴 BLOCKER | §2.4.8 方案 A++ Word-Atomic Payload + Seqlock Double-Read（v1.4 最终方案，v1.3 方案 A+ 已被 R15 取代）+ v1.5 Design-R16 logicalDropBoundary 派生量（v1.7 命名统一） | 源码审查：Producer 仅写 `head` + `buffer[i].payload`（逐 word atomic）+ `buffer[i].seq`，不写 `tail`；不变量 `tail ≤ head`（INV1）+ `head - logicalDropBoundary ≤ Capacity`（INV2，v1.5 Design-R16，v1.7 命名统一）+ slot epoch invariant（INV4）+ word-atomic invariant（INV5）；memory-order release/acquire 正确；linearization point = `slot.seq` 第二次原子读（s2）；Drop Oldest 真正实现（Consumer seqlock double-read 校验跳过被覆写 slot）；C++20 data-race free（所有 word+seq 均原子）；详见 §2.4.8 方案 A++ |
...
| Design-R11（SPSC_DATA Drop-Oldest producer-only head advance 逻辑错误，v1.2 修正 / v1.3 Design-R14 进一步修正 linearizability / v1.4 Design-R15 修正 Slot Lifetime Race / v1.5 Design-R16 修正 Capacity Invariant / v1.7 命名统一） | 🔴 BLOCKER | §2.4.8 方案 A++ Word-Atomic Payload + Seqlock Double-Read（v1.4 最终方案）+ v1.5 Design-R16 logicalDropBoundary 派生量（v1.7 命名统一） | 源码审查：Producer 仅写 `head` + `buffer[i].payload`（逐 word atomic）+ `buffer[i].seq`，不写 `tail`；不变量 `tail ≤ head`（INV1）+ `head - logicalDropBoundary ≤ Capacity`（INV2，v1.5 Design-R16，v1.7 命名统一）；Drop Oldest 测试最旧被覆写（Consumer seqlock double-read 校验失败跳过）+ 新事件保留 + `cumulativeDropCount++`（INV7b observational metric）；memory-order release/acquire；ABA uint64 单调递增不回绕；overwrite race Consumer 读 payload 为逐 word atomic load（无 data race）+ double-seq 覆写校验；详见 §2.4.8 方案 A++ |
...
| Design-R16（Drop-Oldest Queue Logical Boundary / Capacity Invariant Contradiction — INV2 `head - tail ≤ Capacity` 在 Drop-Oldest 模式下不成立，v1.5 修正 / v1.7 命名统一） | 🔴 BLOCKER | §2.4.8 方案 A++ 不变量废弃 INV2-OLD + 引入 logicalDropBoundary 派生量（`max(tail, head - Capacity)`，v1.7 命名统一，v1.5 原名 effectiveTail）+ 新 INV2/INV6/INV7a/INV7b + 区间分解（v1.7 替代旧 INV7/INV8）+ Consumer tryPop 加入 logicalDropBoundary 对齐步骤 + R16 修复子节（核心数学问题回答 + logicalDropBoundary 无竞态证明 + 五种 Consumer-pause 状态 Q1～Q5 + 区间分解证明） | 源码审查：INV2-OLD `head - tail ≤ Capacity` 已废弃标注；新 INV2 `head - logicalDropBoundary ≤ Capacity` 恒成立（数学恒等式）；INV6 `logicalDropBoundary = max(tail, head - Capacity)` 派生量定义；INV7a `pendingDropped = logicalDropBoundary - tail`（瞬时派生量，correctness invariant）；INV7b `cumulativeDropCount` 独立累计统计量（observational metric，非 correctness invariant）；区间分解 `[0, head) = [0, logicalDropBoundary) ∪ [logicalDropBoundary, tail) ∪ [tail, head)`（替代旧 INV8）；Consumer tryPop 含 logicalDropBoundary 对齐步骤（`ct < logicalDropBoundary` 时 `tail.store(logicalDropBoundary)`）；logicalDropBoundary 为派生量非存储变量（无 stale-boundary race）；五种 Consumer-pause 状态 Q1～Q5 穷举完备；TEST-R16-LOGICAL-DROP-BOUNDARY Consumer 长期暂停 + Producer 高频写入 + 恢复后 logicalDropBoundary 对齐 + INV2 恒成立 + 区间分解（CI nightly）；R16 INV2 运行时断言（DEBUG build）；**旧 INV7/INV8 已从 Verification Matrix 中彻底删除（v1.7）** |
...
| Design-R17（effectiveTail / Drop-Oldest logical semantics + accounting consistency — v1.5 INV7/INV8 概念混淆，v1.6 修正 / v1.7 彻底闭环） | 🔴 BLOCKER | §2.4.8 R17 修复子节（R17-A 概念分离：`logicalDropBoundary` / `pendingDropped` / `cumulativeDropCount` 三量分离 + INV7a/INV7b 替代旧 INV7 + R17-B 区间分解 `[0, head) = [0, logicalDropBoundary) ∪ [logicalDropBoundary, tail) ∪ [tail, head)` 替代旧 INV8 + `cumulativeDropCount` 降级为 observational metric）+ §2.4.8 Contract/不变量列表/Verification Matrix 彻底删除旧 INV7/INV8（v1.7 Design-R17-CONSISTENCY） | 源码审查：`pendingDropped` 为派生量（非存储）+ `cumulativeDropCount` 为 `std::atomic<uint64_t>`（`droppedOldestCount` 重命名）+ 两者不混用；**旧 INV7 `droppedCount = effectiveTail - tail` 已从 Contract/不变量列表/Verification Matrix 中彻底删除（v1.7）**，仅在 Amendment v1.5/v1.6 变更记录中保留；**旧 INV8 `head = droppedCount + tail + (head - effectiveTail)` 已从 Contract/不变量列表/Verification Matrix 中彻底删除（v1.7）**，替换为区间分解，仅在 Amendment v1.5/v1.6 变更记录中保留；**`cumulativeDropCount` 不参与 correctness 证明（observational metric，v1.7 Design-R17-OBSERVATIONAL）**，Verification Matrix 中不要求 `cumulativeDropCount` 满足任何 correctness 条件；TEST-R17-CONCEPT-SEPARATION 反例验证（Producer 写 300 → pendingDropped=44, cumulativeDropCount=44；Consumer 消费 100 → pendingDropped=0, cumulativeDropCount=44 不变）；TEST-R17-INTERVAL-DECOMPOSITION 区间分解验证（`tail ≤ logicalDropBoundary ≤ head` + `head - logicalDropBoundary ≤ Capacity` + `[logicalDropBoundary, tail)` 为空）（CI nightly） |
| Design-R18（Drop-Oldest linearization under derived logicalDropBoundary + concurrent head advancement — v1.5 未给出完整线性化证明，v1.6 修正 / v1.7 P1-P8 明确四要素 + Epoch Validity Linearization Theorem） | 🔴 BLOCKER | §2.4.8 R18 修复子节（R18-1 Drop 发生时间统一定义：定义 A Producer-side logical drop 为规范定义，B/C 为 Consumer 发现机制 + R18-2 完整 Push/Pop 线性化证明 P1～P8：Producer LP=`Lp`(seq.store release) / Consumer LP=`Lc`(s2 seq.load acquire) / logicalDropBoundary 对齐 LP=`L_align` / **每种情况明确四要素：Producer LP / Consumer LP / Drop LP（语义发生点） / Consumer 最终接受/拒绝原因（v1.7 Design-R18-FOUR-ELEMENTS）** + R18-3 核心结论：`Lp < Lc` ⇒ old item cannot be returned；`Lc < Lp` ⇒ old item may be returned legitimately + **Theorem (Epoch Validity Linearization)（v1.7 Design-R18-THEOREM）** + R18-4 与 v1.5 logicalDropBoundary 无竞态证明的关系：变量层 vs 语义层） | 文档审查：R18-1 Drop 定义统一（定义 A 规范，B/C 机制）明确；R18-2 P1～P8 八种 interleaving 穷举完备，每种明确四要素（Producer LP / Consumer LP / Drop LP / Consumer 接受/拒绝原因）；R18-3 核心结论形式化明确；**Theorem (Epoch Validity Linearization) 形式化证明明确**（已失效 epoch 不得被 Consumer 在 Pop LP 后返回；仍有效 epoch 可合法返回）；R18-4 变量层（v1.5 已解决）vs 语义层（v1.6 补充）层次分离；TEST-R18-LINEARIZATION 60s TSan + Consumer 暂停注入触发 P5/P6/P7 + `Lp < Lc` 旧 item 不返回 + `Lc < Lp` 旧 item 可返回 + item 序号严格递增；TEST-R18-DROP-DEFINITION 定义 A 与机制 B/C 一致性验证（CI nightly） |
...
> **Amendment v1.7（Contract Consistency Cleanup + Final Proof Closure，受控修订，不推倒 v1.6 主体，不继续扩充理论）**：修复大G 项目经理 Design Gate 终审发现的 v1.6 文档内部前后矛盾 —— Design-R17-CONSISTENCY（§2.4.8 Contract/不变量列表/Verification Matrix 彻底删除旧 INV7 `droppedCount = effectiveTail - tail` 和旧 INV8 `head = droppedCount + tail + (head - effectiveTail)`，不只是加注释，旧定义仅在 Amendment v1.5/v1.6 变更记录中保留作为历史记录）；Design-R17-NAMING（全文正文统一使用 logicalDropBoundary（替代 effectiveTail）/ pendingDropped（替代 droppedCount 瞬时量语义）/ cumulativeDropCount（替代 droppedOldestCount 累计量语义），旧名仅在 Amendment v1.5/v1.6 变更记录 + R17/R18 修复子节的历史回顾中保留）；Design-R17-OBSERVATIONAL（cumulativeDropCount 明确为 observational metric 不参与 correctness 证明，Verification Matrix 中不要求 cumulativeDropCount 满足任何 correctness 条件，重写统计恒等式证明为区间分解证明 + cumulativeDropCount observational metric 声明）；Design-R18-FOUR-ELEMENTS（R18 P1-P8 每种情况明确四要素：Producer LP / Consumer LP / Drop LP（语义发生点）/ Consumer 最终接受/拒绝原因）；Design-R18-THEOREM（增加 Theorem (Epoch Validity Linearization) 形式化证明：已失效 epoch 不得被 Consumer 在 Pop LP 后返回；仍有效 epoch 可合法返回）；Design-VERIFICATION-MATRIX-CONSISTENCY（Verification Matrix 与最终 Contract 逐项一致，不引用已废弃的 INV7/INV8，不引用旧名 effectiveTail）；Design-KEYWORD-AUDIT（文档末尾增加 Keyword Audit 子节）；Design-CONTRACT-SINGLE-SEMANTICS（重写最终 Contract 使全文只有一套 Queue State Model，Contract 中的不变量列表只包含 INV1～INV6 + INV7a + INV7b + 区间分解，旧 INV7/INV8 完全消失）。新增 §2.6.8 Design-R17-CONSISTENCY/NAMING/OBSERVATIONAL + Design-R18-FOUR-ELEMENTS/THEOREM + Design-VERIFICATION-MATRIX-CONSISTENCY + Design-KEYWORD-AUDIT + Design-CONTRACT-SINGLE-SEMANTICS 修订回映 + §2.8 Keyword Audit 子节。

---

## 2.8 Keyword Audit（v1.7 Design-KEYWORD-AUDIT 新增）

本节明确审计旧关键词的残留位置，确保 Contract / Invariant 列表 / Verification Matrix / 代码示例 / 测试设计中不出现旧关键词。

### 2.8.1 旧关键词合法出现位置

| 旧关键词 | 合法出现位置 | 不应出现位置 | 审计结果 |
|---------|------------|------------|---------|
| `droppedCount` | 仅在 Amendment v1.5/v1.6 变更记录中（作为历史记录，标注"已废弃"）+ R17 修复子节的历史回顾中（"v1.5 定义 INV7 `droppedCount = effectiveTail - tail`"等历史回顾语句）+ R17 反例分析中（"v1.5 错误: INV7 droppedCount = effectiveTail - tail = 0"历史回顾） | Contract / Invariant 列表（INV7a/INV7b）/ Verification Matrix / 代码示例 / 测试设计中不出现 | ✓ Contract 中已彻底删除旧 INV7 `droppedCount = effectiveTail - tail`（v1.7 Design-R17-CONSISTENCY），仅保留 ~~旧 INV7~~ 删除线标注 + 历史记录说明 |
| `INV7`（旧定义） | 仅在 Amendment v1.5/v1.6 变更记录中（已废弃说明）+ R17 修复子节的历史回顾中（"v1.5 定义 INV7"等）+ Contract 中 ~~旧 INV7~~ 删除线标注（明确标注"v1.7 彻底废弃"） | Contract / Invariant 列表正文中不作为活跃不变量出现（活跃不变量为 INV7a/INV7b）/ Verification Matrix 中不引用旧 INV7 | ✓ Contract 中旧 INV7 已用删除线标注 + "v1.7 彻底废弃"说明，Invariant 列表活跃不变量为 INV7a（pendingDropped）+ INV7b（cumulativeDropCount observational），Verification Matrix 引用 INV7a/INV7b |
| `INV8`（旧定义） | 仅在 Amendment v1.5/v1.6 变更记录中（已废弃说明）+ R17 修复子节的历史回顾中（"v1.5 INV8 代数恒等式"等）+ Contract 中 ~~旧 INV8~~ 删除线标注（明确标注"v1.7 彻底废弃"） | Contract / Invariant 列表正文中不作为活跃不变量出现（活跃不变量为区间分解）/ Verification Matrix 中不引用旧 INV8 | ✓ Contract 中旧 INV8 已用删除线标注 + "v1.7 彻底废弃"说明，Invariant 列表活跃不变量为区间分解，Verification Matrix 引用区间分解 |
| `effectiveTail` | 仅在 Amendment v1.5/v1.6 变更记录中（作为历史记录，v1.7 已重命名为 logicalDropBoundary）+ R17/R18 修复子节的历史回顾中（"v1.5 原名 effectiveTail，v1.7 全文统一重命名"等历史回顾语句）+ R17 反例分析中（"v1.5 错误: INV7 droppedCount = effectiveTail - tail = 0"历史回顾）+ R18-4 历史回顾中（"v1.5 的 effectiveTail 无竞态证明"等） | Contract / Invariant 列表 / Verification Matrix / 代码示例 / 测试设计 / R16 修复子节当前定义 / 五种 Consumer-pause 状态分析当前定义 / 区间分解证明当前定义中不出现 | ✓ Contract / Invariant 列表 / Verification Matrix / 代码示例 / 测试设计中已统一使用 logicalDropBoundary（v1.7 Design-R17-NAMING），R16 修复子节 / 五种 Consumer-pause 状态分析 / 区间分解证明中已统一使用 logicalDropBoundary |

### 2.8.2 新关键词统一使用位置

| 新关键词 | 定义 | 出现位置 | 审计结果 |
|---------|------|---------|---------|
| `logicalDropBoundary` | `max(tail, head - Capacity)`，派生量（非存储），logical oldest boundary | Contract（INV2/INV6）/ Invariant 列表 / Consumer tryPop 代码 / R16 修复子节当前定义 / 五种 Consumer-pause 状态分析 / 区间分解证明 / Verification Matrix / 测试设计 | ✓ 全文正文统一使用（v1.7 Design-R17-NAMING） |
| `pendingDropped` | `logicalDropBoundary - tail`，瞬时派生量，随 Consumer 推进 tail 而归零 | Contract（INV7a）/ Invariant 列表 / R17 修复子节 / Verification Matrix / 测试设计 | ✓ 全文正文统一使用（v1.7 Design-R17-NAMING） |
| `cumulativeDropCount` | `droppedOldestCount` 运行时值，累计统计量（`std::atomic<u64>`），observational metric 非 correctness invariant | Contract（INV7b）/ Invariant 列表 / R17 修复子节 / Verification Matrix / 测试设计 | ✓ 全文正文统一使用（v1.7 Design-R17-NAMING + Design-R17-OBSERVATIONAL） |

### 2.8.3 Keyword Audit 结论

> **v1.7 Contract Consistency Cleanup 闭环**：
> 1. **旧 INV7 `droppedCount = effectiveTail - tail` 已从 Contract / Invariant 列表 / Verification Matrix 中彻底删除**（v1.7 Design-R17-CONSISTENCY），仅在 Amendment v1.5/v1.6 变更记录 + R17 修复子节历史回顾 + Contract 中 ~~删除线标注~~ 中保留作为历史记录。✓
> 2. **旧 INV8 `head = droppedCount + tail + (head - effectiveTail)` 已从 Contract / Invariant 列表 / Verification Matrix 中彻底删除**（v1.7 Design-R17-CONSISTENCY），替换为区间分解，仅在 Amendment v1.5/v1.6 变更记录 + R17 修复子节历史回顾 + Contract 中 ~~删除线标注~~ 中保留作为历史记录。✓
> 3. **全文正文统一使用 logicalDropBoundary / pendingDropped / cumulativeDropCount**（v1.7 Design-R17-NAMING），旧名 effectiveTail / droppedCount 仅在 Amendment v1.5/v1.6 变更记录 + R17/R18 修复子节历史回顾中保留。✓
> 4. **cumulativeDropCount 明确为 observational metric，不参与 correctness 证明**（v1.7 Design-R17-OBSERVATIONAL），Verification Matrix 中不要求 cumulativeDropCount 满足任何 correctness 条件。✓
> 5. **全文只有一套 Queue State Model**（v1.7 Design-CONTRACT-SINGLE-SEMANTICS），Contract 中的不变量列表只包含 INV1～INV6 + INV7a（pendingDropped）+ INV7b（cumulativeDropCount observational）+ 区间分解，旧 INV7/INV8 完全消失。✓
> 6. **Verification Matrix 与最终 Contract 逐项一致**（v1.7 Design-VERIFICATION-MATRIX-CONSISTENCY），每一行引用最终 Contract 中的具体不变量编号（INV1～INV6 + INV7a + INV7b + 区间分解），不引用已废弃的 INV7/INV8，不引用旧名 effectiveTail。✓
> 7. **R18 P1-P8 每种情况明确四要素**（v1.7 Design-R18-FOUR-ELEMENTS）：Producer LP / Consumer LP / Drop LP（语义发生点）/ Consumer 最终接受/拒绝原因。✓
> 8. **Theorem (Epoch Validity Linearization) 形式化证明明确**（v1.7 Design-R18-THEOREM）：已失效 epoch 不得被 Consumer 在 Pop LP 后返回；仍有效 epoch 可合法返回。✓

> 待用户审查确认后，本文档状态由 DRAFT v1.7 转为 FROZEN 并授权进入下一阶段。
